...
$req_devis = "SELECT count(id_devis) as nb FROM elfamosodevi_devis";
...
<head>
    ...
    <title>Devis - Création facture</title>
    <link type="text/css" rel="stylesheet" href="style.css"/>
    <script src="../js/jquery-3.4.1.min.js"></script>
	 <script src="../js/add-devis.js"></script>
</head>
...
<div id="contenu">
    <?php while($prd_client = $ps_client->fetch()) { ?>
        <?php // while($prd_devis = $ps_devis->fetch()) { ?>
            ...
            <!--        <tr>
                        <td>N°: <input value="D<?php echo date("Y") ?>/<?= $prd_devis['nb'] + 1 ?>"></td>
                    </tr> -->
                    <tr>
                    ...
                    <span id="gerard" class="gege">
                        <tr class="prestation 0">
                            <td><input type="text" name="presta_nom" class="presta_nom 0" placeholder="Intitulé de la Prestation"></td>
                            <td><input type="number" name="presta_prix" class="presta_prix 0" placeholder="Prix">€</td>
                            <td><input type="number" name="presta_quantite" class="presta_quantite 0" placeholder="Quantité"></td>
                    ...
                    <tr>
                        ...
                        <td></td>
                        <td></td>
                        <td><input disabled="disabled" type="text" name="montant_tht" id="montant_tht" placeholder="0,00"> €</td>
                    </tr>
                    <tr>
                        <td>TVA (20%)</td>
                        <td></td>
		...
		<input type="hidden" name ="presta"id="presta" value="">
		<input type="hidden" name ="converted_array" id="converted_array" value="">


                <br><br><br>
                <button type="button" class="btn-refresh_montant">Refresh Montant</button>
                <button class="btn-submit_devis" type="button">Générer un pdf</button>
         
                <button class="btn btn-success" id="submit" name="submit" type="submit" target="_blank"  formaction="../pdfgen.php">Ouvrir le PDF</button>
